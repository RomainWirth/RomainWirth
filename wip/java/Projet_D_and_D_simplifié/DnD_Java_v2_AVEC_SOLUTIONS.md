...
**Cahier de formation complet - Version avec solutions**
...
10. [Itération 9 - Interface graphique JavaFX en Bonus](#itération-9--interface-graphique-javafx--bonus)
...
> - Chaque itération contient : objectifs, specs, tâches, blocs architecture, placeholders UML, et solution.
> - **Java 17+** : sections optionnelles pour apprenants intermédiaires.
> - **Tests JUnit** : sections optionnelles à partir de l'itération 3.
> - **Solution** : blocs repliables en fin de chaque section.
> - Une version sans solutions est disponible séparément (version apprenant).
...
<details>
<summary>✅ Solution - Itération 1</summary>
...
public class Main {
    public static void main(String[] args) {
        ...
        while (cartonsRestants > 0) {
            int chargesVoyage = Math.min(cartonsRestants, capaciteCamion);
            System.out.println("Voyage " + voyage + " : " + chargesVoyage + " cartons chargés.");
            cartonsRestants -= chargesVoyage;
            voyage++;
        }
        System.out.println("Déménagement terminé en " + (voyage - 1) + " voyages.");
    }
}
```
...
**Sortie attendue :**
```
Voyage 1 : 9 cartons chargés.
Voyage 2 : 9 cartons chargés.
Voyage 3 : 9 cartons chargés.
Voyage 4 : 7 cartons chargés.
Déménagement terminé en 4 voyages.
```
...
<details>
<summary>✅ Solution - Itération 2</summary>

**Structure de packages recommandée :**
```
fr.campus.dungeon.characters  → Personnage
fr.campus.dungeon.equipment   → EquipementOffensif, EquipementDefensif
fr.campus.dungeon.game        → Game
fr.campus.dungeon.ui          → Menu
fr.campus.dungeon             → Main
```

**Exemple `toString()` de Personnage :**
```java
@Override
public String toString() {
    return "[" + type + "] " + nom
        + " | PV: " + niveauVie
        + " | Force: " + forceAttaque
        + " | Équip: " + equipOffensif.getNom();
}
```

**Boucle de jeu dans `Game` :**
```java
public void jouerUnTour(Scanner sc) {
    int de = new Random().nextInt(6) + 1;
    positionJoueur += de;
    if (positionJoueur > 64) positionJoueur = 64;
    System.out.println("Case " + positionJoueur + " / 64");
}
```
...
<details>
<summary>✅ Solution - Itération 3</summary>
...
public abstract class Personnage {
    ...
    private EquipementOffensif equipOffensif;

    public Personnage(String nom, int niveauVie, int forceAttaque) {
        this.nom          = nom;
        this.niveauVie    = niveauVie;
        this.forceAttaque = forceAttaque;
    }
    // getters / setters...
    public void subirDegats(int degats) { niveauVie -= degats; }
...
public class Guerrier extends Personnage {
    public Guerrier(String nom) {
        super(nom, new Random().nextInt(6) + 5, new Random().nextInt(6) + 5);
    }

    @Override
    public String toString() {
        return "⚔️ Guerrier [" + getNom() + "] PV:" + getNiveauVie() + " ATK:" + getForceAttaque();
    }
}
...
public class PersonnageHorsPlateauException extends RuntimeException {
    public PersonnageHorsPlateauException(int position) {
        super("Position " + position + " dépasse la case finale (64).");
    }
}
```
...
<details>
<summary>✅ Solution - Itération 4</summary>
...
public abstract class Ennemi implements Case {
    ...
    public Ennemi(String nom, int pv, int atk) {
        this.nom = nom; this.pointsDeVie = pv; this.attaque = atk;
    }
    public boolean estVivant()           { return pointsDeVie > 0; }
    public void subirDegats(int degats)  { pointsDeVie -= degats; }
}

// Orc - cible uniquement les Guerriers
public class Orc extends Ennemi {
    public Orc() { super("Orc", 10, 6); }

    @Override
    public void interaction(Personnage p) {
        if (!(p instanceof Guerrier)) {
            System.out.println("L'Orc ignore " + p.getNom() + " (pas un guerrier).");
            return;
        }
        System.out.println("Un Orc attaque " + p.getNom() + " !");
        // logique de combat complète en IT.6
    }
}

// Initialisation du plateau dans Game
private void initialiserPlateau() {
    plateau = new ArrayList<>(Collections.nCopies(65, new CaseVide()));
    plateau.set(3,  new Gobelin());  plateau.set(6,  new Gobelin());
    plateau.set(45, new Dragon());   plateau.set(52, new Dragon());
    plateau.set(2,  new Massue());   plateau.set(19, new Epee());
    // ... etc.
}
```
...
<details>
<summary>✅ Solution - Itération 5</summary>

```java
public class HeroRepository {
    private static final String URL  = "jdbc:mysql://localhost:3306/DonjonsEtDragons";
    private static final String USER = "root";
    private static final String PASS = "votreMotDePasse";

    private Connection getConnection() throws SQLException {
        return DriverManager.getConnection(URL, USER, PASS);
    }

    public void createHero(Personnage p) {
        String sql = "INSERT INTO hero (type, nom, niveau_vie, force_attaque) VALUES (?,?,?,?)";
        try (var conn = getConnection();
             var ps   = conn.prepareStatement(sql)) {
            ps.setString(1, p.getClass().getSimpleName().toUpperCase());
            ps.setString(2, p.getNom());
            ps.setInt(3, p.getNiveauVie());
            ps.setInt(4, p.getForceAttaque());
            ps.executeUpdate();
    ...
    public void changeLifePoints(int id, int nouveauxPV) {
        String sql = "UPDATE hero SET niveau_vie = ? WHERE id = ?";
        try (var conn = getConnection();
             var ps   = conn.prepareStatement(sql)) {
            ps.setInt(1, nouveauxPV);
            ps.setInt(2, id);
            ps.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Erreur BDD : " + e.getMessage());
...
- Gérer les états de fin de partie
...
5. Fin : ennemi mort (victoire) ou joueur mort (Game Over).
...
- [ ] Fin de partie gérée (victoire case 64, défaite PV ≤ 0)
- [ ] PV mis à jour en BDD après chaque combat

---

<details>
<summary>✅ Solution - Itération 6</summary>

```java
// Extrait combat tour par tour dans Ennemi.interaction()
@Override
public void interaction(Personnage joueur) {
    System.out.println("\n⚔️ Combat : " + joueur.getNom() + " vs " + nom);
    Scanner sc = new Scanner(System.in);

    while (estVivant() && joueur.getNiveauVie() > 0) {
        System.out.println(joueur.getNom() + " PV:" + joueur.getNiveauVie()
                         + "  |  " + nom + " PV:" + pointsDeVie);
        System.out.println("[1] Attaquer  [2] Fuir");

        int choix = sc.nextInt();
        if (choix == 2) {
            int recul = new Random().nextInt(6) + 1;
            System.out.println(joueur.getNom() + " fuit et recule de " + recul + " cases.");
            // Game doit gérer le recul - passer une référence à Game ou utiliser un événement
            return;
        }

        int degats = joueur.getForceAttaque()
                   + joueur.getEquipOffensif().getNiveauAttaque();
        subirDegats(degats);
        System.out.println(joueur.getNom() + " inflige " + degats + " dégâts.");

        if (estVivant()) {
            joueur.subirDegats(attaque);
            System.out.println(nom + " contre-attaque : " + attaque + " dégâts.");
        } else {
...
<details>
<summary>✅ Solution - Itération 7</summary>
...
private void initialiserPlateau() {
    ...
    // Ennemis
    for (int i = 0; i < 10; i++) objets.add(new Gobelin());
    for (int i = 0; i < 10; i++) objets.add(new Sorcier());
    for (int i = 0; i < 4;  i++) objets.add(new Dragon());

    // Caisses surprises
    for (int i = 0; i < 5; i++) objets.add(new Massue());
    for (int i = 0; i < 4; i++) objets.add(new Epee());
    for (int i = 0; i < 5; i++) objets.add(new Eclair());
    for (int i = 0; i < 2; i++) objets.add(new BouleDeFeu());
    for (int i = 0; i < 6; i++) objets.add(new PotionStandard());
    for (int i = 0; i < 2; i++) objets.add(new GrandePotion());

    Collections.shuffle(objets, new Random());

    // Plateau initialisé avec des CaseVide (index 0 inutilisé, cases 1-64)
    plateau = new ArrayList<>(Collections.nCopies(65, (Case) new CaseVide()));
    for (int i = 0; i < objets.size(); i++) {
        plateau.set(i + 1, objets.get(i));
    }
}
```
...
<details>
<summary>✅ Solution - Itération 8</summary>

```java
// Méthode ajouterEquipement() dans Personnage
public void ajouterEquipement(EquipementOffensif e, Scanner sc) {
    if (inventaire.size() < 2) {
        inventaire.add(e);
        System.out.println(e.getNom() + " ajouté à l'inventaire.");
        return;
    }
    System.out.println("Inventaire plein ! Remplacer ?");
    System.out.println("[1] " + inventaire.get(0).getNom()
        + "  [2] " + inventaire.get(1).getNom() + "  [3] Ignorer");
    int choix = sc.nextInt();
    if      (choix == 1) inventaire.set(0, e);
    else if (choix == 2) inventaire.set(1, e);
    else                 System.out.println("Équipement ignoré.");
}

// Coup de tonnerre
public class CoupDeTonnerre implements Case {
    @Override
    public void interaction(Personnage p) {
        p.setCoupDeTonnerreActif(true);
...
<details>
<summary>✅ Solution - Itération 9</summary>

```java
// PlateauController.java - exemple partiel
public class PlateauController {
    @FXML private GridPane grillePlateau;
    @FXML private Label    pvLabel;
    @FXML private Label    forceLabel;
    @FXML private Button   btnLancerDe;
    ...
    @FXML
    private void onLancerDe() {
        game.jouerUnTour(null); // null = pas de Scanner en mode GUI
        rafraichirPlateau();
        pvLabel.setText("PV : " + game.getJoueur().getNiveauVie());
    }
    ...
    private void rafraichirPlateau() {
        ...
        for (int i = 1; i <= 64; i++) {
            ...
            rect.setFill(getCouleurCase(i));
            grillePlateau.add(rect, (i - 1) % 8, (i - 1) / 8);
        }
    }

    private Color getCouleurCase(int index) {
        if (index == game.getPositionJoueur())      return Color.DODGERBLUE;
        Case c = game.getPlateau().get(index);
        if (c instanceof Ennemi)                    return Color.CRIMSON;
        if (c instanceof EquipementOffensif)        return Color.GOLD;
        if (c instanceof Potion)                    return Color.LIMEGREEN;
...
│   ├── Arme.java  /  Sort.java
│   ├── Massue.java  /  Epee.java  /  Arc.java
│   ├── Eclair.java  /  BouleDeFeu.java  /  Invisibilite.java
│   └── Potion.java  /  PotionStandard.java  /  GrandePotion.java  /  CoupDeTonnerre.java
├── enemies/
│   ├── Ennemi.java              (abstraite)
│   ├── Gobelin.java  /  Sorcier.java  /  Dragon.java
│   ├── Orc.java  /  MauvaisEsprit.java
│   └── CaseVide.java
├── game/
│   ├── Case.java                (interface)
│   └── Game.java
...
│   └── Menu.java
└── exceptions/
    ├── PersonnageHorsPlateauException.java
...
*Fin du document - Version avec solutions*

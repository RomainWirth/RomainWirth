...
- Une fonction définie **hors de `main()`** est accessible dans tout le fichier (voire le package).
- `fmt.Print()` / `fmt.Println()` / `fmt.Printf()` sont des fonctions du package `fmt` ; `fmt.Scan()` lit une entrée utilisateur dans le terminal.

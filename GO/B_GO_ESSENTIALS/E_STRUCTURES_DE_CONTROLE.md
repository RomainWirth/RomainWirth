...
- Les **structures de contrôle** permettent de diriger le flux d'exécution selon des conditions.
- `if condition { ... }` exécute un bloc si la condition est vraie (pas de parenthèses autour de la condition en Go).
- `else if condition { ... }` enchaîne une nouvelle condition si la précédente est fausse.
- `else { ... }` est le cas par défaut déclenché si aucune condition précédente n'est vraie.

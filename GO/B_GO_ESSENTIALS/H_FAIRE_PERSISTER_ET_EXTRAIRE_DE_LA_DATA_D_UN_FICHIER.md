...
_N.B. : le underscore `_` utilisé comme deuxième variable de retour des fonctions `.ReadFile()` et `.ParseFloat()` est utilisé ici pour éviter de traiter l'erreur potentielle du retour des fonctions. Dans un code de production, il est recommandé de toujours gérer les erreurs explicitement (voir fichier `I_ERROR_HANDLING`)._

---

## Résumé

- Le package **`os`** (bibliothèque standard) permet d'interagir avec le système de fichiers.
- `os.WriteFile(nomFichier, []byte(contenu), 0644)` **crée ou écrase** un fichier texte.
  - `[]byte()` convertit une `string` en tableau d'octets, seul type accepté par `WriteFile`.
  - `0644` est la permission Unix du fichier (lecture/écriture pour le propriétaire, lecture seule pour les autres).
- `os.ReadFile(nomFichier)` **lit le contenu** d'un fichier et le retourne en `[]byte`.
  - `string(data)` reconvertit les octets en `string` lisible.
- Le package **`strconv`** permet de convertir des `string` en types numériques : `strconv.ParseFloat(str, 64)` retourne un `float64`.
- Ces deux fonctions retournent toujours **deux valeurs** : la donnée et une éventuelle erreur.
- L'identifiant `_` (blank identifier) permet d'ignorer une valeur de retour, mais c'est à éviter en production.

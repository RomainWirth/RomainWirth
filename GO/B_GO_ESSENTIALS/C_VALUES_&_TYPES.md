...
_Les limites de la fonction `fmt.Scan()` :_
...
Mais cette fonction a une limitation importante : **elle s'arrête au premier espace**. Il n'est donc pas possible de récupérer facilement une chaîne contenant plusieurs mots (ex. : un prénom + nom).
Pour lire une ligne complète (avec espaces), on utilisera plutôt `bufio.NewReader` :
```Go
import (
  "bufio"
  "fmt"
...
fmt.Print("Quel est ton nom complet ? ")
name, _ := reader.ReadString('\n') // lit jusqu'au retour chariot
...
- Go est un **langage fortement typé** : toute valeur a un type, et les opérations entre types différents sont interdites (ex. : `int` × `float64` → erreur de compilation).
- **Types de base** : `int`, `float64`, `string`, `bool` ; types de niche : `uint`, `int32`, `int64`, `rune`, etc.
- Chaque type a une **valeur nulle** par défaut (`0`, `0.0`, `""`, `false`).
- **Déclaration de variable** : syntaxe explicite `var x int = 5` ou concise `x := 5` (type déduit automatiquement).
- `const` définit une constante immuable ; peut être déclarée hors de toute fonction (scope global), mais à utiliser avec parcimonie.
- **Conversion de type** : `float64(monInt)` permet de changer le type d'une valeur.
- `fmt.Printf()` formate l'affichage avec des verbes : `%v` (valeur brute), `%f` (float), `%.2f` (2 décimales), `%s` (string).
- `fmt.Sprintf()` retourne une `string` formatée (sans afficher), utile pour stocker le résultat.
- `fmt.Scan()` lit l'entrée utilisateur mais **s'arrête au premier espace** ; utiliser `bufio.NewReader` pour lire des lignes complètes.

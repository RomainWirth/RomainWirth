...
func doubleNumbers(numbers *[]int) []int {
	...
	for _, num := range *numbers {
		dNumbers = append(dNumbers, double(num))
	}
...
Imaginons, dans notre exemple, qu'on souhaite utiliser soit la fonction `double`, soit `triple` selon le nombre du tableau numbers :
```Go
...
```
Le code ci-dessus est loin de refléter un use case réel, mais il illustre bien le principe : une fonction peut retourner une autre fonction, que l'on peut ensuite stocker dans une variable et passer en paramètre.  
En pratique, cette approche est utile lorsqu'on doit sélectionner dynamiquement une stratégie de traitement selon un contexte (ex. : appliquer une remise différente selon le profil d'un client).

---

## Résumé

- **Fonctions comme valeurs** : en Go, les fonctions sont des valeurs de première classe — elles peuvent être passées en paramètre ou retournées par d'autres fonctions.
- Syntaxe pour passer une fonction en paramètre : `func name(param type, fn func(typeParam) typeReturn) typeReturn`
- **Type de fonction** : on peut créer un alias `type myFn func(int) int` pour améliorer la lisibilité, surtout pour les signatures complexes.
- **Retourner une fonction** : une fonction peut retourner une autre fonction — utile pour sélectionner dynamiquement une stratégie de traitement.

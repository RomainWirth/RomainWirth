...
```
Ce fichier étant situé à la racine, il fera partie du même package `main`.
Il faudra bien entendu importer le package `"fmt"` afin d'utiliser la fonction `.Println()` dans le fichier.

> **Bonne pratique** : séparer le code en fichiers distincts dès que la fonction `main()` dépasse une centaine de lignes, ou dès qu'un groupe de fonctions forme une responsabilité cohérente (affichage, I/O, calculs...).

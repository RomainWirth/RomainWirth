...
- **Convention de nommage** : interface à une seule méthode → ajouter `er` au nom de la méthode (`Saver`, `Displayer`).
- Une interface est un **type** en Go : elle peut être utilisée partout où un type est attendu (paramètre, valeur de retour, variable).

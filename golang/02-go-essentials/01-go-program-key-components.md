...
- Un programme Go est organisé en **packages** : chaque fichier débute par `package <nom>`.
- Le **package `main`** est obligatoire : il désigne le point d'entrée de l'application.
- La **fonction `main()`** est la première fonction exécutée ; il ne peut en exister qu'une par application.
- La commande `go mod init <module-path>` crée le fichier `go.mod` qui identifie le module.
- `go build` compile l'application en binaire exécutable ; `go run .` l'exécute directement sans créer de binaire.
- Le package **`fmt`** fait partie de la bibliothèque standard et fournit les fonctions d'affichage (`Println`, `Print`, `Printf`, etc.).
- Un projet peut comporter plusieurs packages organisés en sous-dossiers, importés les uns dans les autres.

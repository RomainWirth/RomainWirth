...
```
le résultat sera un tableau avec le premier élément `"livre"`, le second et le dernier sont vides, et le troisième `"Jeu vidéo"`.
...
- Accès à un élément : `prices[index]` ; modification : `prices[index] = nouvelleValeur`.
- Taille fixe : on ne peut pas ajouter d'éléments au-delà de la taille déclarée.
- Valeur nulle de chaque élément = valeur nulle du type (`0`, `""`, `false`…).
- Un `array` peut contenir n'importe quel type, y compris des `struct`.

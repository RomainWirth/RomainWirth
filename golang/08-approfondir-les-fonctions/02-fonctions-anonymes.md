...
- Une **fonction anonyme** est une fonction sans nom, définie directement là où elle est attendue - comparable à une callback.
- Elle doit correspondre au type de fonction attendu par la fonction qui la reçoit en paramètre.
- Utiliser une fonction anonyme plutôt qu'une fonction nommée quand la logique n'a pas besoin d'être réutilisée.
- **Closure** : une fonction anonyme peut capturer les variables de son environnement (son scope externe) - c'est le principe de fermeture.
- `createTransformer(factor)` est une **fabrique de fonctions** : elle retourne une fonction anonyme qui mémorise la valeur de `factor` (closure).
- Grâce aux closures, on peut générer dynamiquement autant de variantes de fonctions qu'on le souhaite.
```

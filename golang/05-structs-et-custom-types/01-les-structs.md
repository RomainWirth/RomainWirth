...
L'écriture correcte techniquement serait la suivante :
```Go
func outputUserDetails(u *user) {
	fmt.Println((*u).firstName, (*u).lastName, (*u).birthDate)
}
...
> Go applique automatiquement le déréférencement lors de l'accès aux champs d'un struct via pointeur. C'est pourquoi `u.firstName` fonctionne même quand `u` est un `*user`.

...
class Vehicule {
    constructor(_registration, _type) {
...
class VehiculeFleet {
    ...
    addVehicule(vehicule) {
        if (vehicule.type === VehiculeType.BUS) {
            this._buses.addVehicule(vehicule);
        }
        else if (vehicule.type === VehiculeType.CAR) {
            this._cars.addVehicule(vehicule);
        }
    }
    rentVehicule(vehiculeType) {
    ...
    rentSingleVehicule(registration) {
        ...
        if (vehiculeToRent.type === VehiculeType.BUS) {
            this._buses.rentVehicule(vehiculeToRent);
        }
        else if (vehiculeToRent.type === VehiculeType.CAR) {
            this._cars.rentVehicule(vehiculeToRent);
        }
    }
    getVehiculeRegistration(registration) {
        var _a;
        return (_a = this.getVehicules().find(v => v.registration === registration)) !== null && _a !== void 0 ? _a : null;
    }
}
...
vehiculeFleet.displayFleet();
const selectListElement = document.querySelector("#vehicule-list");
function createVehiculeList() {
...
const rentButtonElement = document.querySelector("#rent-button");

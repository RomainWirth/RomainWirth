...
> `ATTENTION` : depuis TypeScript 6.0, les options `"module": "amd"` (ainsi que `"umd"` et `"system"`) et `outFile` sont dépréciées, et seront entièrement supprimées en TypeScript 7.0 (le compilateur réécrit en Go). Le code ci-dessus fonctionne toujours aujourd'hui, mais génère un avertissement de dépréciation à la compilation. Il est possible de le faire taire temporairement en ajoutant `"ignoreDeprecations": "6.0"` dans les `compilerOptions` du `tsconfig.json`, mais ce n'est qu'un pont temporaire : cette combinaison de réglages ne sera plus utilisable du tout à partir de TypeScript 7.0, et il n'existe pas de remplacement direct au sein de TypeScript lui-même. Le seul moyen de continuer à obtenir un fichier unique à partir de plusieurs fichiers compilés sera de passer par un bundler externe (esbuild, Vite, Webpack, Rollup), en combinaison avec des modules ES plutôt que des namespaces. Cette section reste utile pour comprendre du code existant ou ancien, mais l'approche par namespaces n'a plus vraiment d'avenir dans un nouveau projet.
...
- Depuis TypeScript 6.0, `"module": "amd"/"umd"/"system"` et `outFile` sont dépréciés et seront supprimés en TypeScript 7.0. Cette approche fonctionne encore aujourd'hui (avec un avertissement, contournable via `"ignoreDeprecations": "6.0"`), mais n'a pas d'avenir dans un nouveau projet.
